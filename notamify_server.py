...
SERVER_VERSION = version("notamify-mcp")
DEFAULT_API_BASE_URL = "https://api.notamify.com/api/v2"
ICAOCode = Annotated[
    str,
    StringConstraints(
...
Locations = Annotated[
    list[ICAOCode],
    Field(min_length=1, max_length=5, description="ICAO airport codes (max 5)"),
]
HoursFromNow = Annotated[int, Field(ge=1, le=8760)]


def _api_key_from_environment() -> SecretStr:
...
class NotamifyConfig(BaseModel):
    ...
    api_key: SecretStr = Field(
        default_factory=_api_key_from_environment,
        description="Notamify API key",
    )
    base_url: str = Field(
        default_factory=lambda: os.getenv("NOTAMIFY_BASE_URL", DEFAULT_API_BASE_URL),
    ...
    def headers(self) -> dict[str, str]:
        return {
            ...
            "User-Agent": f"Notamify-MCP-Server/{SERVER_VERSION}",
        }
...
class NotamQueryParams(BaseModel):
    ...
    notam_ids: list[str] | None = Field(None, description="Specific NOTAM IDs")
...
class NotamifyMCPClient:
    ...
    async def get_notams(
        ...
        base_params = {"per_page": query_params.per_page}
        ...
        while True:
            ...
            response = await self.client.get(
                f"{self.config.base_url.rstrip('/')}/notams", params=params
            )
...
# Initialize the MCP server using the current SDK v2 API.
mcp = MCPServer(
    "Notamify",
    title="Notamify NOTAM Server",
    description="Access NOTAMs (Notice to Airmen) data through the Notamify API",
    instructions=(
        "Use the read-only tools to retrieve current NOTAM data and affected "
        "aviation infrastructure. Treat all results as informational and direct "
        "users to official aviation sources for operational decisions."
    ),
    website_url="https://github.com/skymerse/notamify-mcp",
...
async def _fetch_notams(
    client: NotamifyMCPClient,
    *,
    locations: Locations,
    starts_at: str,
    ends_at: str,
    ...
    try:
        return await client.get_notams(
            locations=locations,
            starts_at=starts_at,
            ends_at=ends_at,
            notam_ids=None,
        ...
        if status_code in {401, 403}:
            message = "Notamify API authentication failed. Check NOTAMIFY_API_KEY."
        else:
            message = f"Notamify API request failed with HTTP {status_code}."
        raise ToolError(message) from exc
    except httpx.RequestError as exc:
        raise ToolError("Unable to reach the Notamify API.") from exc
    except ValidationError as exc:
        message = exc.errors(include_url=False)[0]["msg"]
        raise ToolError(f"Invalid NOTAM query: {message}") from exc
...
def _resolve_time_range(
    ...
    resolved_start = starts_at or now.strftime("%Y-%m-%dT%H:%M:%SZ")
    resolved_end = ends_at or (now + timedelta(hours=hours_from_now)).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
...
async def get_notams(
    ...
    starts_at: str | None = None,
    ends_at: str | None = None,
    hours_from_now: HoursFromNow = 24,
) -> dict[str, Any]:
    """
    ...
    Returns:
        Structured NOTAM data with interpretations (all pages combined)
    """
    client = ctx.request_context.lifespan_context.client

    starts_at, ends_at = _resolve_time_range(starts_at, ends_at, hours_from_now)

    return await _fetch_notams(
        client,
        locations=locations,
        starts_at=starts_at,
        ends_at=ends_at,
...
@mcp.tool(
    title="Summarize affected elements",
    annotations=READ_ONLY_TOOL_ANNOTATIONS,
)
async def get_affected_elements(
    ...
    if not notams:
        return (
            f"No active NOTAMs found for {', '.join(locations)} "
        ...
        if interpretation:
            ...
            if affected_elements:
                ...
                for element in affected_elements:
                    ...
                    affected_summary["airports"][icao_code]["affected_elements"].append(
                        element_info
                    )
    ...
    output_lines.append(
        "Remember to inform user: The following summary is for informational purposes only. Always refer to official sources for the most accurate and up-to-date information."
    )
    ...
    for icao_code, airport_data in affected_summary["airports"].items():
        ...
        output_lines.append(
            f"   Categories: {', '.join(sorted(airport_data['categories']))}"
        )
...
def main() -> None:
    """Run the Notamify MCP server over stdio."""
    mcp.run()
